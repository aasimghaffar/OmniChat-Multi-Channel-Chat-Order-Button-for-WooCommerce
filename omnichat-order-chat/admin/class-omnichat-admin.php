 ...
 * @subpackage OmniChat_Order_Chat/admin
 */
...
class OmniChat_Admin {
	...
	/**
	 * Enqueue admin styles only on OmniChat screens.
	 *
	 * @param string $hook Current admin page hook.
	 */
	public function enqueue_styles( $hook ) {
		if ( false === strpos( $hook, 'omnichat' ) ) {
			return;
		}

		wp_enqueue_style(
			$this->plugin_name,
			OMNICHAT_PLUGIN_URL . 'admin/css/omnichat-admin.css',
			array(),
			$this->version,
			'all'
		);
	...
	/**
	 * Enqueue admin scripts only on OmniChat screens.
	 *
	 * @param string $hook Current admin page hook.
	 */
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_script(
			$this->plugin_name,
			OMNICHAT_PLUGIN_URL . 'admin/js/omnichat-admin.js',
			array( 'jquery' ),
			$this->version,
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'nonce'   => wp_create_nonce( 'omnichat_admin_nonce' ),
				'siteUrl' => home_url( '/' ),
				'i18n'    => array(
					'confirmRecovered' => __( 'Mark this cart as recovered?', 'omnichat-order-chat' ),
					'confirmApiSend'   => __( 'Send the recovery message to this customer now via the Meta API?', 'omnichat-order-chat' ),
					'sending'          => __( 'Sending…', 'omnichat-order-chat' ),
					'sent'             => __( 'API ✓', 'omnichat-order-chat' ),
					'error'            => __( 'Something went wrong. Please try again.', 'omnichat-order-chat' ),
				),
			)
		);
	}
	...
	public function add_plugin_admin_menu() {
		add_menu_page(
			__( 'OmniChat WhatsApp', 'omnichat-order-chat' ),
			__( 'OmniChat', 'omnichat-order-chat' ),
			'manage_options',
			'omnichat-settings',
		...
		add_submenu_page(
			'omnichat-settings',
			__( 'Widget & Settings - OmniChat', 'omnichat-order-chat' ),
			__( 'Widget & Settings', 'omnichat-order-chat' ),
			'manage_options',
			'omnichat-settings',
			array( $this, 'display_settings_page' )
		);
		...
		add_submenu_page(
			'omnichat-settings',
			__( 'Cart Recovery Log & Analytics - OmniChat', 'omnichat-order-chat' ),
			__( 'Cart Recovery Log', 'omnichat-order-chat' ),
			'manage_options',
			'omnichat-recovery',
		...
		add_submenu_page(
			'omnichat-settings',
			__( 'Order Alerts Hub - OmniChat', 'omnichat-order-chat' ),
			__( 'Order Alerts Hub', 'omnichat-order-chat' ),
			'manage_options',
			'omnichat-orders',
	...
	public function get_default_settings() {
		return OmniChat_Activator::get_default_settings();
	}
	...
	public function sanitize_settings( $input ) {
		if ( ! current_user_can( 'manage_options' ) ) {
			return get_option( 'omnichat_settings', array() );
		}
		...
		$output['woo_btn_text']         = sanitize_text_field( $input['woo_btn_text'] ?? __( 'Order via WhatsApp', 'omnichat-order-chat' ) );
	...
	public function add_action_links( $links ) {
		$settings_link = '<a href="' . esc_url( admin_url( 'admin.php?page=omnichat-settings' ) ) . '">' . esc_html__( 'Settings', 'omnichat-order-chat' ) . '</a>';
		$recovery_link = '<a href="' . esc_url( admin_url( 'admin.php?page=omnichat-recovery' ) ) . '" style="color:#16a34a;font-weight:bold;">' . esc_html__( 'Cart Recovery Log', 'omnichat-order-chat' ) . '</a>';
	...
	public function add_order_whatsapp_action_btn( $order ) {
		...
		$options  = wp_parse_args( get_option( 'omnichat_settings', array() ), $this->get_default_settings() );
		...
		printf(
			'<a class="button wc-action-button wc-action-button-whatsapp omnichat-order-action-btn" href="%1$s" target="_blank" rel="noopener noreferrer" title="%2$s">%3$s</a>',
			esc_url( $wa_url ),
			esc_attr__( 'Send WhatsApp Order Notification', 'omnichat-order-chat' ),
	...
	public function ajax_mark_recovered() {
		check_ajax_referer( 'omnichat_admin_nonce', 'nonce' );

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied.', 'omnichat-order-chat' ) ) );
		}

		$cart_id = isset( $_POST['cart_id'] ) ? absint( wp_unslash( $_POST['cart_id'] ) ) : 0;
		if ( ! $cart_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid cart ID.', 'omnichat-order-chat' ) ) );
		}

		global $wpdb;
		$table_name = $wpdb->prefix . 'omnichat_abandoned_carts';
		...
		if ( false === $updated ) {
			wp_send_json_error( array( 'message' => __( 'Database update failed.', 'omnichat-order-chat' ) ) );
		}

		wp_send_json_success( array( 'message' => __( 'Cart marked as recovered!', 'omnichat-order-chat' ) ) );
	}
	...
	public function ajax_meta_send_test() {
		...
		if ( strlen( OmniChat_Meta_API::normalize_phone( $phone ) ) < 7 ) {
			wp_send_json_error( array( 'message' => __( 'Please enter a valid phone number in international format.', 'omnichat-order-chat' ) ) );
		}

		$api = new OmniChat_Meta_API();

		if ( ! $api->is_configured() ) {
			wp_send_json_error( array( 'message' => __( 'Select "Meta WhatsApp Cloud API" mode and save your token and Phone Number ID first.', 'omnichat-order-chat' ) ) );
		}

		$options  = wp_parse_args( get_option( 'omnichat_settings', array() ), OmniChat_Activator::get_default_settings() );
		...
		if ( '' !== $template ) {
			$result = $api->send_template( $phone, $template, $options['meta_template_lang'], array( __( 'there', 'omnichat-order-chat' ), home_url( '/' ) ) );
		} else {
		...
		wp_send_json_success(
			array(
				'message' => sprintf(
					...
					__( 'Message accepted by Meta (ID: %s). Check the recipient\'s WhatsApp.', 'omnichat-order-chat' ),
	...
	public function ajax_send_recovery_now() {
		...
		global $wpdb;
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom plugin table.
		$cart = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}omnichat_abandoned_carts WHERE id = %d", $cart_id ) );

		if ( ! $cart ) {
			wp_send_json_error( array( 'message' => __( 'Cart not found.', 'omnichat-order-chat' ) ) );
		}

		$api = new OmniChat_Meta_API();

		if ( ! $api->is_configured() ) {
			wp_send_json_error( array( 'message' => __( 'Meta Cloud API mode is not configured.', 'omnichat-order-chat' ) ) );
		}
		...
		wp_send_json_success( array( 'message' => __( 'Recovery message sent via Meta API.', 'omnichat-order-chat' ) ) );
	}
	...
	public function display_settings_page() {
		...
		$options = wp_parse_args( get_option( 'omnichat_settings', array() ), $this->get_default_settings() );
		require OMNICHAT_PLUGIN_DIR . 'admin/partials/omnichat-admin-settings.php';
	}
	...
	public function display_recovery_page() {
		...
		$options = wp_parse_args( get_option( 'omnichat_settings', array() ), $this->get_default_settings() );
		require OMNICHAT_PLUGIN_DIR . 'admin/partials/omnichat-admin-recovery.php';
	}
	...
	public function display_orders_page() {
		...
		$options = wp_parse_args( get_option( 'omnichat_settings', array() ), $this->get_default_settings() );
		require OMNICHAT_PLUGIN_DIR . 'admin/partials/omnichat-admin-orders.php';
	}
}
