(function($) {
  ...
  $(document).ready(function() {
    ...
    var $triggerBtn = $('#omnichat-trigger-btn');
    var $popup = $('#omnichat-popup');
    var $closeBtn = $('#omnichat-close-btn');

    if ($triggerBtn.length && $popup.length) {
      $triggerBtn.on('click', function(e) {
        e.preventDefault();
        $popup.toggleClass('omnichat-hidden');
      });

      $closeBtn.on('click', function(e) {
        e.preventDefault();
        $popup.addClass('omnichat-hidden');
      });

      $(document).on('click', function(e) {
        if (!$(e.target).closest('#omnichat-widget-container').length) {
          $popup.addClass('omnichat-hidden');
        }
      });
    }
    ...
    $(document).on('click', '.omnichat-track-click', function() {
      var agent = $(this).data('agent') || 'Default';
      var product = $(this).data('product') || '';

      // GA4
      if (typeof gtag === 'function' && typeof omnichatVars !== 'undefined' && omnichatVars.ga4Enabled) {
        gtag('event', 'whatsapp_click', {
          'event_category': 'OmniChat',
          'event_label': product ? ('Product: ' + product) : ('Agent: ' + agent)
        });
      }

      // Meta Pixel
      if (typeof fbq === 'function' && typeof omnichatVars !== 'undefined' && omnichatVars.fbEnabled) {
    ...
    if (typeof omnichatVars !== 'undefined' && omnichatVars.ajaxUrl && omnichatVars.captureEnabled) {
      ...
      function captureCheckoutData() {
        ...
        if (phone.length >= 4 || (email.length >= 5 && email.indexOf('@') !== -1)) {
          ...
          $.ajax({
            url: omnichatVars.ajaxUrl,
            type: 'POST',
            data: {
              action: 'omnichat_capture_checkout',
              nonce: omnichatVars.nonce,
              phone: phone,
              email: email,
              name: fullName,
              cart_token: omnichatVars.cartToken || '',
