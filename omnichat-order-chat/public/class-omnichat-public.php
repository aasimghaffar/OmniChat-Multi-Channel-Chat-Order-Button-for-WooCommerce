 ...
 * @subpackage OmniChat_Order_Chat/public
 */
...
class OmniChat_Public {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->options     = wp_parse_args( get_option( 'omnichat_settings', array() ), OmniChat_Activator::get_default_settings() );
	}
	...
	public function maybe_set_cart_token_cookie() {
		...
		if ( ! empty( $_COOKIE['omnichat_cart_token'] ) ) {
			$this->session_token = sanitize_text_field( wp_unslash( $_COOKIE['omnichat_cart_token'] ) );
			return;
		}

		$this->session_token = md5( uniqid( 'omnichat_', true ) . wp_rand() );
	...
	public function enqueue_styles() {
		wp_enqueue_style(
			$this->plugin_name,
			OMNICHAT_PLUGIN_URL . 'public/css/omnichat-public.css',
			array(),
			$this->version,
			'all'
		);
	...
	public function enqueue_scripts() {
		wp_enqueue_script(
			$this->plugin_name,
			OMNICHAT_PLUGIN_URL . 'public/js/omnichat-public.js',
			array( 'jquery' ),
			$this->version,
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'nonce'           => wp_create_nonce( 'omnichat_public_nonce' ),
	...
	public function ajax_capture_checkout() {
		check_ajax_referer( 'omnichat_public_nonce', 'nonce' );
		...
		if ( function_exists( 'WC' ) && WC()->cart && ! WC()->cart->is_empty() ) {
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $values ) {
				...
				if ( $_product ) {
					...
					$cat_name = ( $terms && ! is_wp_error( $terms ) ) ? $terms[0]->name : __( 'General', 'omnichat-order-chat' );
		...
		if ( empty( $cart_items ) ) {
			$cart_items[] = array(
				'name'     => __( 'Store Product', 'omnichat-order-chat' ),
				'quantity' => 1,
				'price'    => 0,
		...
		$existing = $wpdb->get_row( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}omnichat_abandoned_carts WHERE cart_token = %s", $cart_token ) );

		if ( ! $existing && ! empty( $phone ) ) {
			$existing = $wpdb->get_row( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}omnichat_abandoned_carts WHERE customer_phone = %s AND cart_status = 'abandoned'", $phone ) );
		}
	...
	public function render_floating_widget() {
		...
		$greeting      = $this->options['widget_greeting'] ?? __( 'Hello! How can we help you today?', 'omnichat-order-chat' );
		$cta_text      = $this->options['button_cta_text'] ?? __( 'Chat with us', 'omnichat-order-chat' );
		$header_title  = $this->options['popup_header_title'] ?? __( 'Customer Support', 'omnichat-order-chat' );
		$header_sub    = $this->options['popup_header_subtitle'] ?? __( 'Typically replies within a few minutes', 'omnichat-order-chat' );
		...
		require OMNICHAT_PLUGIN_DIR . 'public/partials/omnichat-public-widget.php';
	}
	...
	public function render_woo_order_button() {
		...
		$btn_text    = $this->options['woo_btn_text'] ?? __( 'Order via WhatsApp', 'omnichat-order-chat' );
		...
		?>
		<div class="omnichat-woo-button-wrap">
			<a href="<?php echo esc_url( $wa_url ); ?>" target="_blank" rel="noopener noreferrer" class="button omnichat-woo-order-btn omnichat-track-click" style="background-color:<?php echo esc_attr( $btn_color ); ?> !important;border-color:<?php echo esc_attr( $btn_color ); ?> !important;" data-product="<?php echo esc_attr( $product->get_name() ); ?>">
				<svg class="omnichat-woo-btn-icon" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
	...
	public function shortcode_button( $atts ) {
		$args = shortcode_atts(
			array(
				'phone' => $this->options['default_phone'] ?? '',
				'text'  => __( 'Chat on WhatsApp', 'omnichat-order-chat' ),
				'msg'   => __( 'Hi! I have an inquiry.', 'omnichat-order-chat' ),
			),
			$atts,
			'omnichat_button'
		);
		...
		return '<a href="' . esc_url( $wa_url ) . '" target="_blank" rel="noopener noreferrer" class="omnichat-shortcode-btn omnichat-track-click">' . esc_html( $args['text'] ) . '</a>';
	}
}
