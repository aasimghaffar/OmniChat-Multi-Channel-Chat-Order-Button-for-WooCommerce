...
class OmniChat_Meta_API {
	 ...
	 */
	const CRON_HOOK = 'omnichat_recovery_cron';
	...
	public function __construct() {
		$defaults      = OmniChat_Activator::get_default_settings();
		$this->options = wp_parse_args( get_option( 'omnichat_settings', array() ), $defaults );
	}
	...
	private function request( $payload ) {
		if ( empty( $this->options['meta_api_token'] ) || empty( $this->options['meta_phone_number_id'] ) ) {
			return new WP_Error( 'omnichat_not_configured', __( 'Meta API token or Phone Number ID is missing.', 'omnichat-order-chat' ) );
		}
		...
		$api_message = $data['error']['message'] ?? __( 'Unknown Meta API error.', 'omnichat-order-chat' );
		$api_code    = $data['error']['code'] ?? $code;

		return new WP_Error(
			'omnichat_meta_api_error',
			sprintf(
				/* translators: 1: Meta error code, 2: Meta error message. */
				__( 'Meta API error %1$s: %2$s', 'omnichat-order-chat' ),
				$api_code,
				$api_message
			)
		);
	 ...
	 *
	 * @param object $cart Row from the omnichat_abandoned_carts table.
	 * @return array|WP_Error
	 */
	public function send_recovery_for_cart( $cart ) {
		if ( empty( $cart->customer_phone ) ) {
			return new WP_Error( 'omnichat_no_phone', __( 'This cart has no phone number.', 'omnichat-order-chat' ) );
		}

		$customer_name = '' !== trim( (string) $cart->customer_name )
			? $cart->customer_name
			: __( 'there', 'omnichat-order-chat' );
	...
	public function mark_sent( $cart_id ) {
		...
		$wpdb->update(
			$wpdb->prefix . 'omnichat_abandoned_carts',
	...
	public function add_cron_interval( $schedules ) {
		$schedules['omnichat_15min'] = array(
			'interval' => 15 * MINUTE_IN_SECONDS,
			'display'  => __( 'Every 15 minutes (OmniChat)', 'omnichat-order-chat' ),
		);
		return $schedules;
	}
	...
	public function maybe_schedule_cron() {
		...
		if ( $enabled && ! $scheduled ) {
			wp_schedule_event( time() + MINUTE_IN_SECONDS, 'omnichat_15min', self::CRON_HOOK );
